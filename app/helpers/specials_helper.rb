  ...
  def generate_url(type, car)
    ...
    if current_user.id != car.user_id
      ...
      if !@permission.save
        ...
        issue_tokens.push("http://auto-biography.herokuapp.com" + "/specials/?&report_type=#{type.class.name}&issue_id=#{type.id}&car_id=#{car.id}&token=#{@token}")
        issue_tokens.push("http://auto-biography.herokuapp.com" + "/cars/#{car.id}/issues/#{type.id}/?&token=#{@token}")
        issue_tokens
      end
      ...
      if !@permission.save
        ...
        issue_tokens.push("http://auto-biography.herokuapp.com" + "/specials/?&report_type=#{type.class.name}&car_id=#{car.id}&token=#{@token}")
        issue_tokens.push("http://auto-biography.herokuapp.com" + "/cars/#{car.id}/maintenances/?&token=#{@token}")
        issue_tokens
      end
    else
      nil
